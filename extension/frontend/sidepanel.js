    ...
    function displayResults(data) {
        ...
        if (data.textResult) {
            if (data.textResult.error) {
                ...
                // Combine and display fact-check results AND related news in the visible card
                ...
                if (data.textResult.fact_check && Array.isArray(data.textResult.fact_check) && data.textResult.fact_check.length > 0) {
                    combinedSourcesHtml += data.textResult.fact_check.map(createSourceItemHTML).join('');
                }
                if (data.textResult.related_news && Array.isArray(data.textResult.related_news) && data.textResult.related_news.length > 0) {
                    combinedSourcesHtml += data.textResult.related_news.map(createNewsItemHTML).join('');
                }

                if (combinedSourcesHtml) {
                    factCheckResultsContainer.innerHTML = combinedSourcesHtml;
                } else {
                    factCheckResultsContainer.innerHTML = '<div class="text-gray-500 dark:text-gray-400 p-4">No related news or sources found.</div>';
                }
            } else {
        ...
        // (The 'newsResults' HTML card is hidden, but populate it just in case)
        newsResultsContainer.innerHTML = '';
    }
