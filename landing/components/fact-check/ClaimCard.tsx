...
export function ClaimCard({ claimText, status, verdict, confidence, reasoning, citations, index }: ClaimCardProps) {
  return (
    <motion.div
      ...
      className="bg-slate-900/40 backdrop-blur-md border border-slate-800/80 rounded-2xl p-6 shadow-xl relative overflow-hidden group hover:border-slate-700 transition-colors"
    >
      {/* Confidence Bar Background overlay */}
      {status === "done" && confidence !== undefined && (
        <div 
          className="absolute bottom-0 left-0 h-1 bg-gradient-to-r from-blue-600 to-blue-400 opacity-80" 
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Claim #{index + 1}</span>
            {status !== "done" && (
